...
const PASSWORD_SALT = 'sv_salt_zencrypt_2024';
const LEGACY_PASSWORD_SALTS = ['sv_salt_ZenCrypt_2024'];
...
async function verifyStoredPassword(password, storedHash) {
  ...
  if (currentHash === storedHash) {
    return { matched: true, upgradedHash: null };
  }

  for (const legacySalt of LEGACY_PASSWORD_SALTS) {
    const legacyHash = await hashPassword(password, legacySalt);
    if (legacyHash === storedHash) {
      return { matched: true, upgradedHash: currentHash };
    }
  }
...
async function handleLogin() {
  ...
  const passwordCheck = await verifyStoredPassword(pass, users[user].password);
  if (!passwordCheck.matched) { toast('Invalid username or password', 'error'); return; }

  if (passwordCheck.upgradedHash) {
    users[user].password = passwordCheck.upgradedHash;
    saveUsers(users);
  }
...
async function renderPublicUsersList() {
  const adminsGrid = document.getElementById('adminsGrid');
  const usersGrid = document.getElementById('usersGrid');
  if (!adminsGrid || !usersGrid) return;
  ...
  try {
    ...
    if (window.USE_SERVER_API) {
      ...
      usersList = Object.values(localUsers).map(u => ({
        id: u.id || u.username,
        username: u.username,
        role: u.role || 'user',
    ...
    const createCard = (u, isAdmin) => `
      <div class="member-card ${isAdmin ? 'is-admin' : 'is-user'}">
        <div class="member-info">
          <div class="member-avatar">${u.username.substring(0,2).toUpperCase()}</div>
          <div class="member-details">
            <div class="member-name">${escapeHtml(u.username)}</div>
            <div class="member-joined">Joined: ${new Date(u.created_at).toLocaleDateString()}</div>
          </div>
        </div>
    ...
    usersList.forEach(u => {
      if (u.role === 'admin') {
        adminsHtml += createCard(u, true);
      } else {
        usersHtml += createCard(u, false);
      }
    });

    adminsGrid.innerHTML = adminsHtml || '<div class="no-members-msg">No system administrators found.</div>';
    usersGrid.innerHTML = usersHtml || '<div class="no-members-msg">No regular users found.</div>';

  } catch (err) {
    console.error('Error fetching public users:', err);
    adminsGrid.innerHTML = '<div class="no-members-msg" style="color:var(--error-color)">Error loading members</div>';
    usersGrid.innerHTML = '<div class="no-members-msg" style="color:var(--error-color)">Error loading members</div>';
  }
}

async function performInit() {
  renderPublicUsersList();
  if (window.USE_SERVER_API) {
    if (API.token) {
      try {
        ...
        if (currentUser.two_factor_enabled !== undefined) {
          currentUser.twoFactor = currentUser.two_factor_enabled;
        }
...
(async function initServerMode() {
  ...
  handleLogin = async function() {
    ...
    try {
      ...
      // Direct login (no 2FA) — use loginSuccess() so RBAC (admin nav) is applied
  ...
  verify2FA = async function() {
    ...
    try {
      ...
      if (otpTimerInterval) { clearInterval(otpTimerInterval); otpTimerInterval = null; }
      pending2FAUser = null;
      // Use loginSuccess() so RBAC (admin nav) is applied
      loginSuccess(result.user);
      toast('✅ Two-factor verification successful!', 'success');
    } catch (err) {
      otpAttempts++;
